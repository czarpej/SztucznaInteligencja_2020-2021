# -*- coding: utf-8 -*-
"""
Created on Sun Dec  6 12:27:50 2020
...
from bayess import DiscreteNBC
...
def discretize(X, B: int, X_ref=None):  
    ...
    X_d = np.floor((X - mins) / (maxes - mins) * B).astype("int32")  # X dyskretne
...
if __name__ == "__main__":
    ...
    B = 101  # liczba koszykow dyskretyzacyjnych
    X_train_d = discretize(X_train, B)
    X_test_d = discretize(X_test, B, X_ref=X_train)
    
    domain_sizes = np.ones(n, dtype="int32") * B  # rozmiary naszych dziedzin
    
    print(X_test_d.shape)  # ilosc przykladow testowych
    
    laplaces = [False, False, True, True]
    log2uses = [False, True, False, True]
    
    for i in range(len(laplaces)):
        dnbc = DiscreteNBC(domain_sizes=domain_sizes, laplace=laplaces[i], log2use=log2uses[i])
        dnbc.fit(X_train_d, y_train)
        
        print("Ilosc koszykow:", B, "| Poprawka Laplace'a:", laplaces[i], "| Włączone logarytmowanie:", log2uses[i])
